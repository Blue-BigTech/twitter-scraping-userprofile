...
var datafile = fs.readFileSync('./twiterations.json');
...
const usersProfileDetails = async (start) => {
    ...
    try {
        for (let index = start; index < 300; index++) {
            const element = datajson[index].link;
        ...
        await browser.close();
        var start = serials + 1;
        await usersProfileDetails(start);
    }
    ...
    var now = new Date();
    var dateandtime = date.format(now, "hh-mm-ss-A-DD-MM-YYYY");

    const filename = `twitter/${dateandtime}-twitter.xlsx`;
    wb.write(filename);

    console.log("your tasks finished");

    await browser.close();
};

module.exports = {usersProfileDetails}